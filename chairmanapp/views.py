...
def register(request):
    if request.POST:
        ...
        if role=="chairman":
            ...
            cid=Chairman.objects.create(email=email,password=password,is_varify=isvarify,is_active=isactive,role=role,otp="1234",created_at="not provided",updated_at="not provided")
            ...
            if role=="societymember":
                ...
                uid=User.objects.create(email=email,password=password,is_varify=isvarify,is_active=isactive,role=role,otp="1234",created_at="not provided",updated_at="not provided")
                uid.save()

                sid=Societymember.objects.create(user_id=uid,firstname=firstname,lastname=lastname,email=email,contact_no=contact_no)
                sid.save()
                smsg="Added Successfully"
                context={
                        'uid':uid,
